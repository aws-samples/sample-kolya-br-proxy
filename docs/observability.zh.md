...
**`log_level` 和 `metrics_enabled` 支持运行时热修改** — 通过管理 API（`PUT /admin/observability`）修改后，会通过 Redis Pub/Sub 广播到所有 Pod。**`log_format` 和 `tracing_exporter` 需要重启服务**（Kubernetes 中通过 Pod 滚动更新）。
...
| `backend/app/core/config_sync.py` | 跨 Pod 配置同步（Redis Pub/Sub） |
...
### 第 1 步：Metrics — 定位时间范围和模型

在 CloudWatch Metrics Dashboard 上，`RequestDuration` P99 在 14:00–14:30 飙升到 15s（正常约 2s），集中在 `claude-opus-4-6` 模型。
...
使用 CloudWatch Logs Insights 查询这个时间窗口：
...
结果显示 `token_name=lucky` 的几个请求 `duration=15s`，`trace_id=abc123...`，`input=85000` tokens — 超长输入。

### 第 3 步：Traces — 分析完整调用链

在 X-Ray Console 中搜索 `trace_id=abc123...`，查看完整瀑布图：

```
[POST /v1/messages]  ─────────────────────────────── 15.2s
  ├─ http receive                                      0ms
  ├─ http send                                         0ms  (200 + SSE 头)
  ├─ bedrock.invoke_stream (FAILED)     ──── 5.0s     ⚠️
  ...
  │
  ├─ http send × N                                     0ms
  └─ http receive                        ──── 15.1s   (等待断开)
```

### 结论

- **Metrics** 发现异常：特定模型的 P99 延迟飙升
- **Logs** 定位元凶：token `lucky` 发送了 85K token 的超长请求
- **Traces** 分析瓶颈：us-west-1 被限流（429），failover 到 us-east-1 成功但浪费了 5s + 跨区域延迟更高

**对策**：
1. 给 `lucky` token 设日限额，防止大量长请求耗尽区域配额
2. 考虑在 us-east-1 也开 inference profile，降低 failover 延迟

串联三者的关键字段是 `trace_id` — 同时存在于结构化日志和 X-Ray trace 中。
...
- **运行时修改**（通过管理界面或 `PUT /admin/observability`）：通过 Redis Pub/Sub 立即同步到所有 Pod，同时覆盖 uvicorn 的 access/error 日志。
- **启动时配置**（`KBR_LOG_LEVEL`）：仅在启动时由 `configure_logging()` 读取一次，修改后需要重启。
- 有效值：`DEBUG`、`INFO`、`WARNING`、`ERROR`（不区分大小写）
- 如果 Redis 不可用，运行时修改仅对接收 API 请求的那个 Pod 生效（优雅降级）。
