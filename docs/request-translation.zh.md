...
Kolya BR Proxy 如何将请求转换为 AWS Bedrock API 调用，以及如何将响应转换回来。代理支持两种客户端 API 格式：

- **OpenAI 兼容**（`POST /v1/chat/completions`）-- 需要格式转换
- **Anthropic Messages API**（`POST /v1/messages`）-- 近乎直通 Bedrock InvokeModel

Anthropic 模型使用 InvokeModel API（原生 Messages API 格式），非 Anthropic 模型（Nova、DeepSeek、Mistral、Llama 等）使用 Converse API。
...
| 文件 | 在转换中的角色 |
|---|---|
| `api/v1/endpoints/chat.py` | OpenAI 入口点；提取 `X-Bedrock-*` 请求头；编排流式/非流式流程 |
