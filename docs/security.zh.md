...
API Token 在存储前使用 **SHA256** 进行哈希。这是一个简单的确定性哈希，无盐值、无密钥、无迭代。
...
| 字段 | 算法 | 用途 |
|------|------|------|
| `token_hash` | SHA256（无盐值、无密钥） | 认证查找（已建索引） |
| `encrypted_token` | Fernet（AES-128-CBC） | 管理员检索原始 Token |

> **注意**：`hash_token()` 使用普通 SHA256，速度非常快（纳秒级）。API Token 本身具有高熵（`secrets.token_urlsafe(32)`），因此即使使用快速哈希，暴力破解仍然不可行。而 Refresh Token 的哈希使用的是 PBKDF2（见下文"Refresh Token 安全加固"章节），提供更强的离线攻击防护。

### 关键文件

- `backend/app/core/security.py` — `hash_token()`、`verify_token()`、`encrypt_token()`、`decrypt_token()`
- `backend/app/core/redis.py` — `RedisCache` 封装，JSON 序列化，Redis 不可用时优雅降级
- `backend/app/services/token.py` — Token 创建与验证服务
- `backend/app/services/token_cache.py` — `CachedTokenService`，Redis 缓存层（TTL 300s）+ DB 回退
...
系统在 `User` 模型上定义了两个字段用于访问控制：

| 字段 | 类型 | 说明 |
|------|------|------|
| `role` | 枚举 | `super_admin`（完全访问）或 `admin`（受权限限制） |
| `permissions` | JSON | 控制 `admin` 角色可管理的具体资源范围 |

### 权限字段说明

`permissions` JSON 对象支持以下键：

| 权限键 | 取值 | 控制范围 |
|--------|------|----------|
| `manage_api_keys` | `true`/`"all"`、`[uuid, ...]`、`false` | API Token 增删改查、告警规则/通知 |
| `manage_teams` | `true`/`"all"`、`[uuid, ...]`、`false` | 团队增删改查、成员管理 |
| `manage_models` | `true`/`"all"`、`[uuid, ...]`、`false` | 模型配置 |
| `view_usage` | `true`/`false` | 用量统计查看 |
| `view_monitor` | `true`/`false` | 定价表查看 |

权限取值规则：

- `true` 或 `"all"`：对该类型所有资源有完全访问权限
- UUID 数组：仅可访问指定资源（仅 `manage_api_keys`、`manage_teams`、`manage_models` 支持资源级限定）
- `false` 或缺失：无权限（403 Forbidden）
- **特殊情况**：`permissions` 为 `null` 或空对象 `{}` 时，Admin 对所有业务操作拥有全权限（等同于不做权限限制）。一旦 `permissions` 中设置了任何一个键，未设置的权限键将被视为无权限

### Super Admin vs Admin 功能对比

### Entra ID 基于组的访问控制

当 `KBR_MICROSOFT_ENABLE_GROUP_SYNC=true` 时，用户角色和权限从 Azure AD 安全组成员身份解析，而非手动设置。配置详情参见 [OAuth 配置 — Entra ID 组同步](oauth-setup.zh.md#entra-id-组同步)。

| 方面 | Cognito 模式 | Entra ID 组同步模式 |
|------|-------------|-------------------|
| 用户创建 | Super Admin 手动邀请 | 通过组成员身份自动授权 |
| 权限分配 | 在 Admin Users 页面逐用户设置 | 在 Entra Groups 页面按组设置 |
| 权限更新 | 手动编辑 | 每次登录自动覆盖 |
| 角色升降级 | 手动编辑 | 自动——由最高优先级的组映射决定 |
| 权限回收 | 手动停用 | 从所有已映射的 Azure 组中移除（下次登录拒绝） |
| UI 角色编辑 | 允许 | Microsoft 用户禁用（tooltip 提示由组映射管理） |
...
Microsoft OAuth 回调
  │
  ├─ 没有组映射记录 AND 数据库中无 Microsoft 用户？
  │    → Bootstrap：授予此首位用户 super_admin
  │
  ├─ 没有组映射记录 BUT 已存在 Microsoft 用户？
  │    → 拒绝登录（403 "组映射未配置"）
  │
  ├─ 存在组映射 → 调用 Graph API /me/memberOf
  │    │
  │    ├─ Graph API 调用失败（网络、401、429、500）？
  │    │    → 安全关闭：拒绝登录（503 "无法验证组成员身份"）
  │    │
  │    ├─ 用户所属组匹配到映射？
  │    │    → 使用最高优先级映射的 role + permissions
  │    │
  │    └─ 用户所属组不匹配任何映射？
  │         → 拒绝登录（403 "未授权"）
  │
  └─ 将解析出的 role/permissions 覆盖写入用户记录（每次登录都覆盖）
```

#### 关键设计决策

1. **Graph API 失败时安全关闭** — 如果 `/me/memberOf` 调用因任何原因失败，登录直接被 503 拒绝。这防止了在无法验证组成员身份时残留过期的高权限。

2. **Bootstrap 仅限一次** — 仅当数据库中不存在任何 Microsoft 用户且未配置任何组映射时，第一个登录的用户自动获得 `super_admin`。后续用户在首位管理员配置组映射之前都会被拒绝。

3. **每次登录同步权限** — 每次成功登录都会用当前组映射的结果覆盖用户的 role 和 permissions。如果用户在 Entra ID 中被移到不同的组，其 KBP 权限在下次登录时自动更新。组同步启用时无法手动覆盖 Microsoft 用户的角色。

4. **Cognito 用户不受影响** — 组同步仅适用于 Microsoft OAuth 用户。Cognito 用户继续通过 Admin Users 页面手动管理。两种认证方式独立共存。

5. **基于优先级的解析** — 当用户属于多个已映射的组时，`priority` 值最高的映射生效。例如："Engineering" 组 = 受限权限的 admin（priority 5）；"Platform Team" = super_admin（priority 10）。

#### 仅 Super Admin 可用

| 功能 | 端点 | 说明 |
|------|------|------|
| 用户管理 | `/admin/users/*` | 邀请、编辑、停用管理员账号 |
| Entra 组映射 | `/admin/entra-groups/*` | 配置组到角色的映射 |
| 审计日志 | `/admin/audit` | 查看所有操作审计记录和摘要 |
| 定价管理 | `/admin/pricing/*` | 手动更新模型定价、查询定价详情 |
| 可观测性 | `/admin/observability` | 查看和修改 OpenTelemetry 配置 |
| 密钥导出 | `GET /admin/tokens/export/keys` | 以 CSV 格式导出所有 API Key 明文 |

#### 两者都可用（Admin 受 permissions 控制）

| 功能 | 端点 | 权限键 | 资源级限定 |
|------|------|--------|-----------|
| API Key 增删改查 | `/admin/tokens/*` | `manage_api_keys` | 支持（限定到指定 Key） |
| Key 余额调整/撤销 | `/admin/tokens/{id}/adjust`、`/revoke` | `manage_api_keys` | 支持 |
| 告警规则管理 | `/admin/alerts/rules/*` | `manage_api_keys` | — |
| 告警通知查看 | `/admin/alerts/notifications/*` | `manage_api_keys` | — |
| 团队增删改查 | `/admin/teams/*` | `manage_teams` | 支持（限定到指定 Team） |
| 团队成员管理 | `/admin/teams/{id}/members/*` | `manage_teams` | 支持 |
| 模型配置 | `/admin/models/*` | `manage_models` | 支持 |
| 用量统计 | `/admin/usage/*` | `view_usage` | — |
| 定价表查看 | `/admin/monitor/pricing-table` | `view_monitor` | — |
...
|------|------|------|
| 仪表盘 | `/` | 概览页面 |
| 活动记录 | `/admin/audit-logs/activity` | 近期管理操作 |
| Playground | `/playground` | 对话测试 |
| 账号设置 | `/settings` | 个人账号设置 |

### 前端菜单可见性

前端侧边栏根据权限动态过滤菜单项。Admin 未授权的功能不会在菜单中显示：

| 菜单项 | 可见条件 |
|--------|---------|
| Dashboard | 所有管理员 |
| Teams | `manage_teams` 权限 |
| API Keys | `manage_api_keys` 权限 |
| Models | `manage_models` 权限 |
| Playground | 所有管理员 |
| Monitor | `view_monitor` 权限 |
| Activity | 所有管理员 |
| Admin Users | 仅 Super Admin |
| Entra Groups | 仅 Super Admin（组同步模式） |
| Settings | 所有管理员 |
...
| `/admin/entra-groups/*` | 仅 `super_admin` 角色 |
| `/admin/pricing/*` | 仅 `super_admin` 角色 |
| `/admin/observability` | 仅 `super_admin` 角色 |
| `/admin/audit` | 仅 `super_admin` 角色 |
...
| `backend/app/core/security.py` | JWT/API Token 生成、SHA256 哈希、PBKDF2 Refresh Token 哈希、验证、Fernet 加密 |
| `backend/app/core/redis.py` | `RedisCache` 封装（JSON 序列化，Redis 不可用时优雅降级） |
