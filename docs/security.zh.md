...
本文档介绍 BR Enterprise Proxy 的安全防护设计，包括 AWS WAF（ALB 层的限流与托管规则集）、跨域资源共享（CORS）、跨站请求伪造（CSRF）的防护、API Token 哈希、Refresh Token 安全加固以及 JWT 库选型——攻击原理、为什么 API 网关必须防范这些攻击，以及本项目的具体实现。

---

## 为什么 API 网关需要安全防护？

BR Enterprise Proxy 同时服务两类客户端：
...
```
1. 管理员登录 BR Enterprise Proxy 管理面板（浏览器持有 JWT）
...
```
1. 管理员已登录 BR Enterprise Proxy 管理面板（浏览器持有 HttpOnly 刷新令牌 Cookie）
...
API Token 在存储前使用**普通 SHA256** 进行哈希（`hashlib.sha256(token.encode()).hexdigest()`）。这提供了一个快速、确定性的哈希值，用于数据库索引查找。
...
| 字段 | 算法 | 用途 |
|------|------|------|
| `token_hash` | SHA256（普通，无密钥） | 认证查找（已建索引） |
| `encrypted_token` | Fernet（AES-128-CBC） | 管理员检索原始 Token |

### 安全特性

由于 SHA256 速度较快，抗暴力破解能力依赖于 **Token 熵值**而非计算硬度。API Token 使用 `secrets.token_urlsafe(32)` 生成（256 位熵），使穷举搜索在任何哈希速度下都不可行。`token_hash` 字段通过 `secrets.compare_digest()` 实现常数时间查找。如果需要获取原始 Token 值（如在管理面板中显示），`encrypted_token`（Fernet AES，密钥从 `JWT_SECRET_KEY` 派生）提供恢复路径。
...
管理面板实施了两种角色的 RBAC 控制。**`super_admin`** 拥有对所有资源和操作的无限制访问权限。**`admin`** 用户的权限由用户记录上的 `permissions` JSON 字段定义。每个权限键（如 `tokens`、`teams`）可设置为：

| 权限值 | 含义 |
|--------|------|
| `true` 或 `"all"` | 完全访问该类型的所有资源 |
| `["uuid-1", "uuid-2", ...]` | 仅限访问指定的资源 UUID |
| `false` 或缺失 | 无访问权限 |

系统在 `backend/app/api/deps.py` 中使用三个辅助函数：

- **`require_permission(permission)`** — FastAPI 依赖注入工厂；用户缺少指定权限时返回 403。
- **`get_allowed_resource_ids(user, permission)`** — 完全访问时返回 `None`，受限访问时返回允许的 UUID 列表，无权限时返回空列表。
- **`check_resource_scope(user, permission, resource_id)`** — 用户无权访问特定资源时抛出 403。

### 团队与预算管理

团队提供组织分组功能，支持月度消费配额和每日自动限额。团队预算控制确保单个团队不会超出其分配的支出额度，为整个组织提供成本治理能力。
...
| `backend/app/core/security.py` | JWT/API Token 生成、SHA256 哈希、验证、Fernet 加密 |
| `backend/app/core/redis.py` | `RedisCache` 封装（JSON 序列化，Redis 不可用时优雅降级） |
