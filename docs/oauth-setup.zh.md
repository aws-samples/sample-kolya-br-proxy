...
### 通过 `deploy-all.sh` 自动配置（推荐）

使用 `deploy-all.sh` 部署时，Cognito 完全自动配置——无需手动操作 AWS 控制台：

```
deploy-all.sh 步骤1 (Terraform)                步骤1 后置操作                 步骤2-4
────────────────────────────────              ─────────────────────          ─────────
创建：                                        读取 Terraform 输出：           ExternalSecret 同步
  • Cognito 用户池                              • user_pool_id               SM → k8s Secret
  • 应用客户端（机密类型）                        • app_client_id                → Pod 环境变量：
  • 托管 UI 域名                                • app_client_secret              KBR_COGNITO_*
  • 回调 URL（来自 tfvars）                    写入 Secrets Manager
  • 密码策略                                   创建第一个管理员用户
  • Pre-signup Lambda（邮箱域过滤）              （交互式输入邮箱）
```

**部署过程中发生的事：**

1. **Terraform 创建所有资源** — 用户池、应用客户端（授权码模式，`openid`+`profile`+`email` 范围）、托管 UI 域名、回调 URL（从 `terraform.tfvars` 中的 `frontend_domain` 推导）、密码策略、以及验证邮箱域名的 Pre-signup Lambda。
2. **凭证推送到 Secrets Manager** — `deploy-all.sh` 从 Terraform 输出读取 `cognito_user_pool_id`、`cognito_app_client_id`、`cognito_app_client_secret`，写入项目的 Secrets Manager secret。
3. **创建第一个管理员** — 脚本检测到空的用户池，交互式提示输入管理员邮箱，调用 `admin-create-user`，Cognito 发送临时密码到邮箱。
4. **ExternalSecret Operator 同步到 Pod** — k8s 中的 `ExternalSecret` 资源每小时从 Secrets Manager 拉取凭证，挂载为 `KBR_COGNITO_USER_POOL_ID`、`KBR_COGNITO_CLIENT_ID`、`KBR_COGNITO_CLIENT_SECRET` 环境变量。

**部署完成后，只需：**
- 查收邮件中的临时密码
- 登录并设置永久密码（至少 8 个字符，包含大写 + 小写 + 数字 + 特殊符号）

无需手动配置 Cognito。

### 手动配置（不使用 `deploy-all.sh`）

如需手动配置 Cognito（例如使用已有用户池或非 Terraform 部署）：
...
1. 在 [AWS 控制台 > Cognito](https://console.aws.amazon.com/cognito/) 中创建新的用户池。
...
Microsoft OAuth 需要在 Azure 门户手动注册应用（不像 Cognito 由 Terraform 自动创建）。但 `deploy-all.sh` 负责将凭证注入集群：

```
Azure 门户（手动）            deploy-all.sh              Kubernetes
──────────────────           ──────────────             ──────────
应用注册                  →   --configure auth      →    AWS Secrets Manager
  • Client ID                  （交互式输入）              → ExternalSecret
...
2. 填写以下信息：
   ...
   - **重定向 URI**：平台选 `Web`，URI 为 `https://<your-frontend-domain>/auth/microsoft/callback`
     - 本地开发另外添加 `http://localhost:3000/auth/microsoft/callback`
3. 点击 **注册**。
4. 在概览页面记录：
   - **应用程序（客户端）ID**
   - **目录（租户）ID**
...
**方式 A：通过 `deploy-all.sh`（推荐用于生产环境）**

```bash
./deploy-all.sh --configure auth
# 选择 "1) Add/Update Microsoft Entra ID (SSO)"
# 输入：Client ID、Client Secret、Tenant ID
# 脚本写入 Secrets Manager → ExternalSecret 同步到 Pod
```

脚本还会提示是否重启后端 Pod 以加载新的 secret。
...
2. 后端调用 Microsoft Graph API `/me/memberOf` 获取用户的安全组成员身份
3. 将组与 `entra_group_mappings` 表进行匹配（最高优先级优先）
4. 匹配组决定用户的角色和权限（**每次登录覆盖写入**）
5. 无匹配组 → 403 拒绝
6. Graph API 调用失败（网络错误、401、429、500） → 503 拒绝（安全关闭）

### 引导（首次登录）

当 `KBR_MICROSOFT_ENABLE_GROUP_SYNC=true`、数据库中**无组映射且无 Microsoft 用户**时，第一个 Microsoft 登录自动获得 `super_admin`。这解决了鸡生蛋问题。

**引导窗口立即关闭** — 一旦第一个 Microsoft 用户写入数据库，后续登录会被 "组映射未配置" 拒绝，直到 super_admin 在 **Entra Groups** 页面创建映射。

> **重要：** 启用组同步后不要分享登录链接，直到你准备好成为第一个登录者。首次登录会占用 bootstrap super_admin 名额。
...
| 场景 | 结果 |
|------|------|
| 组同步关闭（`false`） | 所有 Microsoft 用户获得 `admin` 角色（旧行为） |
| 组同步开启，无映射，无 MS 用户 | 首位用户获得 `super_admin`（bootstrap） |
| 组同步开启，无映射，已有 MS 用户 | 拒绝登录 — 403 "组映射未配置" |
| 组同步开启，已配置映射 | 用户必须在已映射的组中才能登录 |
| 用户在多个已映射组中 | 最高 `priority` 组的角色/权限生效 |
| 用户不在任何已映射组中 | 拒绝登录 — 403 "未授权" |
| Graph API 不可达 / 返回错误 | 拒绝登录 — 503 "无法验证组成员身份" |
| 用户在 KBP 中已停用 | 无论组成员身份如何均拒绝登录（403） |
| 在 UI 中编辑 Microsoft 用户角色 | 不允许 — 组同步启用时编辑按钮禁用 |
...
| Microsoft 回调返回 403 "组映射未配置" | 组同步已开启但尚未创建映射，且 bootstrap 名额已被占用 |
| Microsoft 回调返回 503 "无法验证组成员身份" | Graph API `/me/memberOf` 调用失败 — 检查网络、token scope、管理员同意 |
| Microsoft 用户角色手动修改后恢复原样 | 正常行为 — 组同步每次登录都覆盖角色；应修改组映射而非手动编辑 |
